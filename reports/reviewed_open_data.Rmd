<style>
    th {  background-color:#E0E0E0 ;
          border-bottom:1px solid black;
          padding:5px;
    }
    td{
        border-bottom:1px dotted black;
        padding:5px;
    }
    table{ 
        border-collapse:collapse;
        margin:auto;
        border: 1px solid black;
    }
</style>

Open Data Services (reviewed)
=============================

Below is the list of products and services published on CloudStore whose description includes either the "open data" or "opendata" keywords **and** that were identified by a reviewer to be genuine users of open data or having a proposition strongly centred around open data.

```{r echo=FALSE, message=FALSE, results='asis', prompt=FALSE, dpi=600}
library(dplyr)
d <- read.csv("./all_open_data_REVIEWED.csv", stringsAsFactors = FALSE, encoding = "UTF-8")
d <- d[ 
        !is.na(d$extra_open_data_level) & d$extra_open_data_level > 0, 
        c('extra_category', 'supplier_name', 'sku', 'name', 'cloudstoreUrl', 'description')
    ] %.% arrange(extra_category, supplier_name, name)
# make the name column into a link to the product url on CloudStore
d$name <- paste0("<a href='", d$cloudstoreUrl, "'>", d$name, "</a>")
# drop the cloudstoreUrl column
d <- d[, !(names(d) %in% c('cloudstoreUrl'))]
# strip \n characters, that would break the table once converted
d$description <- gsub("\\n", " ", d$description)
# rename the columns to be more reader-friendly
names(d) <- c('Category', 'Supplier name', 'Product SKU', 'Product / Service Name', 'Description')
# why do I need the row below?
row.names(d) <- as.integer(row.names(d))
kable(d, format = "markdown", row.names = TRUE)
```

This table was produced using [https://github.com/Digital-Contraptions-Imaginarium/odi-gcloud-analysis](https://github.com/Digital-Contraptions-Imaginarium/odi-gcloud-analysis) on `r as.character(Sys.time())`.

![Small ODI logo](data:image/png;base64,)